...
function renderUI(tree, currentFolderHash, origin) {
  ...
  const breadcrumbsHtml = breadcrumbs.map((b, idx) => {
    ...
    return `
      <a href="/?folder=${b.hash}" class="hover:text-white transition-colors truncate max-w-[100px] md:max-w-[180px]">${escapeHtml(b.name)}</a>
      ...
      const actionButton = isFolder
        ...
        : `
          <div class="flex gap-2">
            <button onclick="copyToClipboard('${origin}/download/${item.hash}', event)" class="bg-white/5 hover:bg-indigo-600/20 hover:text-indigo-300 text-gray-400 p-2 rounded-lg border border-white/5 transition-all duration-150 tooltip" title="Copy Direct Download Link">
    ...
    <body class="min-h-screen text-gray-200 p-4 md:p-8 flex justify-center items-start">
      <div class="w-full max-w-5xl flex flex-col gap-6 mt-4">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 glass rounded-3xl p-6 border border-white/5 shadow-2xl">
          <div>
            <h1 class="text-2xl font-bold bg-gradient-to-r from-indigo-400 via-purple-400 to-cyan-400 bg-clip-text text-transparent">MEGA Cloud Indexer</h1>
            <p class="text-xs text-gray-400 mt-1">Direct downloads and media browsing hosted on Cloudflare Edge</p>
          </div>
          <div class="flex gap-3">
            <a href="/?refresh=1" class="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-500 text-white font-medium text-sm px-4 py-2.5 rounded-xl transition-all duration-150 shadow-lg shadow-indigo-600/20">
